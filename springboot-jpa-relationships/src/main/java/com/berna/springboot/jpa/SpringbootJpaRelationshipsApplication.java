...
import com.berna.springboot.jpa.repositories.CourseRepository;
...
public class SpringbootJpaRelationshipsApplication implements CommandLineRunner {
	...
	public void manyToManyBidireccionalFindById() {
		...
		Student student1 = optStudent1.get();
...
//		student2.setCourses(Set.of(course2));
		...
		//Esto de abajo sería el request. Lo de arriba agrega a la DB
		Optional<Student> optionalStudentDb = studentRepository.findById(3L);
		if(optionalStudentDb.isPresent()) {
			...
			if(courseOptionalDb.isPresent()) {
				...
				// Tengo q pisar el equals en cursos
...
//		student2.setCourses(Set.of(course2));
	...
	@Transactional
	public void manyToManyFindAndRemove() {
		Optional<Student> optStudent1 = studentRepository.findById(1L);
		Optional<Student> optStudent2 = studentRepository.findById(2L);
		...
		//Esto de abajo sería otro request. Lo de arriba agrega a la DB
		Optional<Student> optionalStudentDb = studentRepository.findById(1L);
		if(optionalStudentDb.isPresent()) {
			Student studentDb = optionalStudentDb.get();
